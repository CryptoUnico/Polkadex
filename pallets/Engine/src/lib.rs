...
use sp_runtime::traits::{AtLeast32BitUnsigned, IdentifyAccount, MaybeSerializeDeserialize, Member, Verify, Zero, AccountIdConversion, Saturating};
...
impl<T: Config> Module<T> {
    ...
    fn finalize_commitment(now: T::BlockNumber) -> Weight {
        if now - T::DisputePeriod::get() >= 0.into() && !<FraudProofs<T>>::contains_key(now - T::DisputePeriod::get()) {
            ...
            prev_state.nonce = commitment.final_nonce;
    ...
    fn execute_offchain_worker(now: T::BlockNumber) {
        ...
        if target_block >= 0.into() && <Commitments<T>>::contains_key(target_block) {
            let target_commitment: Commitment<T::AccountId, T::Balance, T::Signature, T::AssetID> = <Commitments<T>>::get(target_block);
            let mut prev_state: State<T::AssetID, T::Balance, T::AccountId> = <Balances<T>>::get();
                ...
                match log {
                    Log::SpotSettlement(maker, taker, fee,  nonce) => {
                        ...
                        if Self::verify_signatures(&maker, &taker, maker_msg, taker_msg) && Self::verify_nonce(nonce, prev_state.nonce) {
                            Self::verify_trades_and_update(&maker, &taker, &prev_state.balances, fee);
                        } else {
    ...
    fn verify_trades_and_update(maker: &SpotTrade<T::AccountId, T::Balance, T::Signature, T::AssetID>,
                                taker: &SpotTrade<T::AccountId, T::Balance, T::Signature, T::AssetID>,
        ...
        match (maker.order_type, taker.order_type) {
            ...
            (OrderType::BidLimit, OrderType::AskLimit) => {
                if taker.price <= maker.price && taker.amount <= maker.amount {
                    ...
                    // Calculate Fees
                    let quote_amt: T::Balance = taker.price*taker.amount; // Given to maker
                    let base_amt: T::Balance = taker.amount - taker.amount*fee; // Given to taker
                    let fees_collected: T::Balance = base_amt*fee;  // Given to exchange
                    // Maker Balances
                    let maker_quote: T::Balance = state.get_mut(&(&maker.trader,&maker.quote_asset)).unwrap();
                    let maker_base: T::Balance = state.get_mut(&(&maker.trader,&maker.base_asset)).unwrap();
                    // Taker Balances
                    let taker_quote: T::Balance = state.get_mut(&(&taker.trader,&taker.quote_asset)).unwrap();
                    let taker_base: T::Balance = state.get_mut(&(&taker.trader,&taker.base_asset)).unwrap();
                    // Exchange Balances
                    let exchange_base: T::Balance = state.get_mut(&(Self::get_exchange_account(),&taker.base_asset));
    ...
    fn verify_state(computed_state: &State<T::AssetID, T::Balance, T::AccountId>,
                    given_state: &State<T::AssetID, T::Balance, T::AccountId>) -> bool {
        unimplemented!();
    }

    fn verify_signatures(maker: &SpotTrade<T::AccountId, T::Balance, T::Signature, T::AssetID>,
                         taker: &SpotTrade<T::AccountId, T::Balance, T::Signature, T::AssetID>,
                         maker_msg: &T::Hash,
